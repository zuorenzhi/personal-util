...
-- SELECT COUNT(DISTINCT user_id) num,DATE_FORMAT(create_time,'%m-%d-%Y') from chat_record
-- where create_time <'2018-04-08' and user_id <> '' GROUP BY DATE_FORMAT(create_time,'%m-%d-%Y')

--用户数，按天去重user_id;后统计多天用户数量时不去重
-- SELECT COUNT(DISTINCT session_id) num,DATE_FORMAT(create_time,'%Y-%m-%d') days from chat_record
...
-- SELECT DATE_FORMAT(create_time,'%Y-%m-%d') AS item_day,COUNT(DISTINCT session_id) AS session_num from chat_record
-- WHERE session_id <> ''
-- and create_time >= '2018-03-19' and create_time < '2018-04-19'
-- GROUP BY DATE_FORMAT(create_time,'%m-%d-%Y')

--用户问题数
-- SELECT DATE_FORMAT(create_time,'%Y-%m-%d') AS item_day,COUNT(question_id) AS session_num from chat_record
-- GROUP BY DATE_FORMAT(create_time,'%m-%d-%Y')
