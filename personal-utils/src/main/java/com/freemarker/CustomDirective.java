...
public class CustomDirective implements TemplateDirectiveModel{
	...
	@Override
	public void execute(Environment env, Map params, TemplateModel[] tplArr,
		...
		if(body==null){  
            ...
            String act = (String)getObjValue(PARAM_ACTION, params);  
            ...
            if(age!=null){  
                env.setVariable("append1", ObjectWrapper.DEFAULT_WRAPPER.wrap("年龄:"+age));
            }  
            if(act!=null){  
            	env.setVariable("append2", ObjectWrapper.DEFAULT_WRAPPER.wrap("行为:"+act));
            }  
	...
	/**
	 * <p>Discription:[获取String类型的参数的值]</p>
	 * Created on 2016年11月21日
	 * @param paramName
	 * @param paramMap
	 * @return
    ...
    public static Object getObjValue(String paramName, Map<String, TemplateModel> paramMap) throws TemplateModelException{  
    	TemplateModel model = paramMap.get(paramName);  
    	if(model == null){  
    		return null;  
    	}  
    	if(model instanceof TemplateScalarModel){  
    		...
    		return ((TemplateDateModel)model).getAsDate().toString();  
    	}else if (model instanceof TemplateHashModel) {  
    		return ((TemplateHashModel)model).get("").toString();  
    	}else{  
    		throw new TemplateModelException(paramName);  
    	}  
    }  
